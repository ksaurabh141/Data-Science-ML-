# -*- coding: utf-8 -*-
"""
Created on Tue Sep  4 22:50:27 2018
...
sns.FacetGrid(data=titanicAll,row='Survived', size=6).map(sns.countplot, 'Age1').add_legend()
titanicAll.groupby(['Survived','Age1']).size()
sns.factorplot(x="Survived", hue="Age1", data=titanicAll, kind="count", size=6)
#In graph there is much deviation in Age1 created, so I will use Title for Model Building
...
#In graph there is much deviation in Title created, so I will use Title for Model Building
...
titanicAll['Family'] = titanicAll['SibSp']+titanicAll['Parch'] + 1
...
#Remember that we don't know which paramaeters for Controlling overfitting is good. We have keep try with different values,
